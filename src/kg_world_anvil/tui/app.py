...
from kg_world_anvil.tui.screens.dedup import DedupScreen
...
class KgApp(App):
    ...
    #ingest-status, #review-status, #graph-status, #query-status, #issues-status, #dedup-status {
        padding: 0 2 1 2;
    }
    #entity-table, #edge-table, #review-table, #query-results, #issues-table, #dedup-table {
        height: 1fr;
        margin: 0 2;
    }
    ...
    BINDINGS = [
        ...
        Binding("d", "show_screen('dedup')", "Dedup"),
        Binding("shift+q", "quit", "Quit"),
    ]
